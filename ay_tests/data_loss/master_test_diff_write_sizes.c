...
#define ITERATIONS 2
#define BLOCK_NUM (STRIDE_END + 1)
#define ADDR_RANGE (BLOCK_NUM * 10)
#define LAST_BLOCK_SIZE  4     // In KB
#define SRAND_RANGE 1000000

// Stride is also used as sleep duration multiplier as a different test
#define STRIDE_START    100*256
#define STRIDE_INC      100*256
#define STRIDE_END      1000*256
...
int main (int argc, char *argv[])
{
    ...
    sprintf_size =  sprintf (buf2,
        "Block Size | Test Type | Iter | Application Wrote | Drive Acknowledged | Persistent Data | Data Corrupted | Application Lost | Iter Time | Loss Time (us) | BW KB/ms\n");
...
//    system(buf2);
	...
	for (block_size = 4096; block_size <= (LAST_BLOCK_SIZE * 1024);) {
       ...
       // check_rec_sanity();
         ...
         */
		for (i = 0, pers_data = 0, corrupt_data = 0; i < stride; i++) {
        ...
        sprintf_size =  sprintf (buf2,
            "%10u |   %c%4d   | %4u | %17u | %18u | %15u | %14u | %16u | %9lu | %lu | %lu", block_size / 1024, rand_flag ? 'R' : 'S', 0, ITERATIONS - iters + 1,
            stride * 4, data_acked, pers_data, ((data_acked - pers_data) > 0) ? (data_acked - pers_data) : 0 ,  corrupt_data, rec[stride -1].time, get_pers_time(data_acked), ((stride * 4 / 1024) * 1000000) / rec[stride -1].time );
